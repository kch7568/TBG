...
public class PostDetailActivity extends AppCompatActivity {
                        ...
                        runOnUiThread(() -> {
                            new AlertDialog.Builder(PostDetailActivity.this)
                                    ...
                                    .setPositiveButton("예", (dialog, which) -> {
                                        ...
                                        new Thread(() -> {
                                            try {
                                                URL url = new URL("http://10.0.2.2:8888/kch_server/DeletePost?postNum=" + postNum + "&sessionId=" + sessionId);
    ...
    private void fetchCurrentUserId(String authorId) {
        new Thread(() -> {
            try {
                ...
                URL url = new URL("http://10.0.2.2:8888/kch_server/GetCurrentUser?sessionId=" + encodedSessionId);
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
    ...
    private void addToFavorites(String postNum, String sessionId) {
        new Thread(() -> {
            try {
                URL url = new URL("http://10.0.2.2:8888/kch_server/FavoriteInteraction");
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestMethod("POST");
                conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
                conn.setDoOutput(true);
    ...
    private void checkFavoriteStatus(String postNum, String sessionId) {
        new Thread(() -> {
            try {
                URL url = new URL("http://10.0.2.2:8888/kch_server/GetFavoriteStatusServlet?postNum=" + postNum + "&sessionId=" + sessionId);
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestMethod("GET");
