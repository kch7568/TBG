...
public class SettingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (sessionId != null) {
            new Thread(() -> {
                try {
                    String serverUrl = "http://10.0.2.2:8888/kch_server/GetAlertStatus";
    ...
    private void updateAlertStatus(String status) {
        new Thread(() -> {
            try {
                String serverUrl = "http://10.0.2.2:8888/kch_server/UpdateAlertStatus";
                URL url = new URL(serverUrl);
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestMethod("POST");
                conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
    ...
    private void deleteAccount(String sessionId) {
        new Thread(() -> {
            try {
                String serverUrl = "http://10.0.2.2:8888/kch_server/DeleteAccount";
                URL url = new URL(serverUrl);
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestMethod("POST");
                conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
